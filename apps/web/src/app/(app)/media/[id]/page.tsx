  ...
  return (
    <>
      <PageHeader
        ...
        action={
          <div className="flex flex-wrap items-center gap-3">
            {canManage && (
              <a
                href={`/media/${album.id}/export`}
                className={buttonVariants({ variant: "outline", size: "sm" }) + " min-h-[44px] gap-2 lg:min-h-0"}
              >
      ...
      <div className="p-4 space-y-6 lg:p-6">
        ...
        {/* Two photos to a row on a phone, three from sm up as before. */}
            ...
            return (
              <figure key={item.id} className="overflow-hidden rounded-lg border bg-card">
                {url ? (
                  ...
                  <img src={url} alt={item.caption ?? ""} className="h-32 w-full object-cover sm:h-40" />
                ) : (
                  <div className="flex h-32 w-full items-center justify-center bg-muted text-xs text-muted-foreground sm:h-40">
                    Preview unavailable
                  </div>
                )}
                {item.caption && (
                ...
                {untagged.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="flex flex-wrap gap-4 rounded-lg border p-3">
                      {/* The preview takes the full row on a phone so the
                          tagging form beside it is never squeezed. */}
                      <div className="w-full shrink-0 sm:w-40">
                        {url ? (
                          ...
                          <img
                            src={url}
                            alt={item.caption ?? ""}
                            className="h-40 w-full rounded object-cover sm:h-28 sm:w-40"
                          />
                        ) : (
                          <div className="flex h-40 w-full items-center justify-center rounded bg-muted text-xs text-muted-foreground sm:h-28 sm:w-40">
                            No preview
                          </div>
                        )}
