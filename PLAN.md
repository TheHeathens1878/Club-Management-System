...
| P0.3 | Set up CI (GitHub Actions): typecheck, tests, `supabase db lint`, migration dry-run against a branch DB | CI green on a no-op PR; required checks configured | ◑ 2026-08-22 — `.github/workflows/ci.yml` has two jobs: `verify` (lint/typecheck/test) and `database` (replays all migrations + seed on a fresh local Postgres, `db lint --fail-on error`, and `db push --dry-run` against prod when `SUPABASE_ACCESS_TOKEN`/`SUPABASE_DB_PASSWORD` secrets exist). PR #1 green on both jobs and merged; repo secrets added; run 32560047110 on `main` green incl. prod dry-run ("Remote database is up to date"). REMAINING: require both checks on `main` (Settings → Branches → rule for `main` → require status checks "Lint, typecheck, test" and "Migrations apply cleanly, db lint, prod dry-run"; recommend also "Require a pull request before merging"). |
