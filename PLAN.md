...
| P0.2 | Link Supabase CLI to project `rwpglslbkhsqyxjhnpue`; pull current schema into versioned migrations as baseline | `supabase db diff` is clean against prod; baseline migration committed | ◑ 2026-08-21 — baseline `supabase/migrations/20260821000000_baseline.sql` reconstructed from live prod catalogs over MCP (`db pull` unavailable in that session) and `packages/db/src/database.types.ts` regenerated. Object counts verified against prod (20 tables / 233 cols / 54 constraints / 7 indexes / 38 policies / 5 functions / 3 triggers). 2026-08-22: CLI linked; `migration repair --status applied 20260821000000` run on prod; `migration list` shows local/remote in sync. REMAINING: `supabase db diff --linked -s public` needs Docker (shadow DB) which is not installed — either install Docker Desktop or verify by executing the baseline on a Supabase preview branch and comparing catalogs. |
