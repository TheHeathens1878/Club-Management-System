...
8. **The Me view is the default when you log in** (2026-08-25, third pass). A sixth role view, "Me", held by anyone linked to a person record; it sits first in the default precedence, so a sign-in with no stored choice lands in it, on the Club Lobby as its main page. Its menu is exactly the parent menu of ruling 7 — **Club**: Club Lobby, My groups, Messaging, Events, Registrations · **Me**: My Profile, Connected Adults, My Children · **Finance**: My Subs · **Safeguarding**: Report a concern · **Settings**: Comms preferences — with each entry still behind its capability gate (My Children only for guardians). The parent view keeps its team-scoped switcher entries; "Me" is the plain, whole-person option at the top of the dropdown, and the wider hats live in the switcher. The middleware's first-visit nudge (already gone by ruling 1) is now removed in code too.
