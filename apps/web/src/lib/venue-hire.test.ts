...
import { bookingCost, sessionsBetween, slotCost } from "./venue-hire";
...
describe("slotCost and bookingCost", () => {
  ...
  it("is the price times the sessions", () => {
    expect(slotCost(booking, { weekday: 1, pricePence: 4500 })).toEqual({ sessions: 4, costPence: 18000 });
    expect(slotCost(booking, { weekday: 1, pricePence: null })).toEqual({ sessions: 4, costPence: null });
  });
  ...
  it("adds a booking up, counting the unpriced slots", () => {
    ...
    ).toEqual({ sessions: 4 + 3 + 3, costPence: 18000 + 18000, unpricedSlots: 1 });
  });
});
