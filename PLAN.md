...
| P0.2 | Link Supabase CLI to project `rwpglslbkhsqyxjhnpue`; pull current schema into versioned migrations as baseline | `supabase db diff` is clean against prod; baseline migration committed | ☑ 2026-08-22 — baseline `supabase/migrations/20260821000000_baseline.sql` reconstructed from live prod catalogs; types regenerated; CLI linked; `migration repair --status applied 20260821000000` on prod; history in sync. Verified by executing the baseline on a throwaway preview branch (`baseline-verify`, deleted after) and diffing catalogs vs prod: columns/constraints/indexes/triggers/policies/RLS/grants/sequences all identical. Known cosmetic residual: prod's 5 functions were authored via the dashboard with CRLF bodies, so `db diff` will show 5 spurious `create or replace function` lines until they are re-saved with LF (do as a no-op migration when convenient). Docker Desktop not installed, so CLI `db diff` itself is still unrun locally. |
